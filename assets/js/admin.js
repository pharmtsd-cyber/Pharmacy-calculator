...
async function handleChangePassword() {
    ...
    if(result.status === "success") { alert("密碼修改成功！"); document.getElementById('pw-modal').classList.add('hidden'); } 
...
function renderLists() {
    ...
    // 渲染藥師清單
    document.getElementById('list-staff').innerHTML = STORE.staff.filter(s => 
        (s.name||'').toLowerCase().includes(fStaff) || String(s.emp_id).includes(fStaff)
    ).map(s => `
        <tr><td>${s.emp_id}</td><td>${s.name}</td><td>${s.role}</td><td><span class="${s.status==='Y'?'text-green-600':'text-red-500'}">${s.status}</span></td>
            <td>${(CURRENT_USER.role === 'Admin' || CURRENT_USER.role === 'Programmer') ? `<button onclick="deleteRecord('deleteStaff', '${s.emp_id}')" class="text-red-500 hover:text-red-700"><i class="fa-solid fa-trash"></i></button>` : ''}</td></tr>
    `).join('');

    // 渲染參數清單
    document.getElementById('list-params').innerHTML = STORE.parameters.filter(p => 
        (p.param_code||'').toLowerCase().includes(fParams) || (p.param_name||'').toLowerCase().includes(fParams)
    ).map(p => `
        <tr><td>${p.param_code}</td><td>${p.param_name}</td><td>${p.default_unit}</td>
            <td><button onclick='editParameter(${JSON.stringify(p).replace(/'/g, "&#39;")})' class="text-blue-500 hover:text-blue-700 mr-2"><i class="fa-solid fa-pen"></i></button><button onclick="deleteRecord('deleteParameter', '${p.param_code}')" class="text-red-500 hover:text-red-700"><i class="fa-solid fa-trash"></i></button></td></tr>
    `).join('');

    // 渲染藥品清單
    document.getElementById('list-drugs').innerHTML = STORE.drugs.filter(d => 
        (d.local_name||'').toLowerCase().includes(fDrugs) || (d.generic_name||'').toLowerCase().includes(fDrugs) || (d.cat_1||'').toLowerCase().includes(fDrugs)
    ).map(d => `
        <tr><td><span class="bg-blue-100 text-blue-800 text-[10px] px-1 rounded">${d.cat_1||''}</span>${d.cat_2 ? `<i class="fa-solid fa-angle-right text-[10px] mx-1 text-gray-400"></i><span class="bg-blue-50 text-blue-800 text-[10px] px-1 rounded">${d.cat_2}</span>` : ''}</td>
            <td><div class="font-bold text-blue-900">${d.local_name} ${d.common_brand?'('+d.common_brand+')':''}</div><div class="text-[10px] text-gray-500">${d.generic_name}</div></td>
            <td><span class="${d.status==='Y'?'text-green-600':'text-red-500'} font-bold">${d.status}</span></td>
            <td><button onclick='editDrug(${JSON.stringify(d).replace(/'/g, "&#39;")})' class="text-blue-500 hover:text-blue-700 mr-2"><i class="fa-solid fa-pen"></i></button><button onclick="deleteRecord('deleteDrug', '${d.drug_id}')" class="text-red-500 hover:text-red-700"><i class="fa-solid fa-trash"></i></button></td></tr>
    `).join('');

    // 渲染公式清單
    document.getElementById('list-formulas').innerHTML = STORE.formulas.map(f => {
        const drug = STORE.drugs.find(d => d.drug_id === f.drug_id);
        return { ...f, drug_name: drug ? drug.local_name : '未知藥品' };
    }).filter(f => 
        (f.drug_name||'').toLowerCase().includes(fFormulas) || (f.formula_name||'').toLowerCase().includes(fFormulas)
    ).map(f => `
        <tr><td class="font-bold">${f.drug_name}</td><td>${f.formula_name}</td><td class="font-mono text-xs text-blue-800 bg-blue-50 p-1 rounded">${f.formula_string}</td>
            <td class="text-xs text-red-600">單:${f.single_max||'--'} ${f.single_max_unit||''}<br>日:${f.daily_max||'--'} ${f.daily_max_unit||''}</td>
            <td><button onclick='editFormula(${JSON.stringify(f).replace(/'/g, "&#39;")})' class="text-blue-500 hover:text-blue-700 mr-2"><i class="fa-solid fa-pen"></i></button><button onclick="deleteRecord('deleteFormula', '${f.formula_id}')" class="text-red-500 hover:text-red-700"><i class="fa-solid fa-trash"></i></button></td></tr>
    `).join('');
}
...
function resetParameterForm() {
    ...
    document.getElementById('param-code').value = ''; document.getElementById('param-code').disabled = false;
...
async function saveParameter() {
    ...
    if(mode === 'add' && STORE.parameters.some(p => p.param_code === code)) return alert("代碼已存在，不可重複新增！");
...
function editDrug(drug) {
    document.getElementById('drug-mode').value = 'edit'; document.getElementById('drug-id').value = drug.drug_id;
    ...
    document.getElementById('drug-local').value = drug.local_name || ''; document.getElementById('drug-brand').value = drug.brand_name || ''; document.getElementById('drug-common-brand').value = drug.common_brand || ''; document.getElementById('drug-generic').value = drug.generic_name || '';
    document.getElementById('drug-ingred').value = drug.ingredients || ''; document.getElementById('drug-status').value = drug.status;
    document.getElementById('btn-save-drug').innerText = "更新儲存"; document.getElementById('btn-cancel-drug').classList.remove('hidden');
    window.scrollTo({ top: 0, behavior: 'smooth' });
}
function resetDrugForm() {
    document.getElementById('drug-mode').value = 'add'; document.getElementById('drug-id').value = '';
    ['cat1','cat2','cat3','local','brand','common-brand','generic','ingred'].forEach(id => document.getElementById(`drug-${id}`).value = '');
...
async function saveDrug() {
    const payload = {
        ...
        ingredients: document.getElementById('drug-ingred').value, status: document.getElementById('drug-status').value
    };
...
function editFormula(f) {
    ...
    document.getElementById('admin-drug-select').value = f.drug_id; document.getElementById('admin-formula-name').value = f.formula_name;
    document.getElementById('admin-result-unit').value = f.result_unit; document.getElementById('admin-remark').value = f.remark || '';
    document.getElementById('formula-single-max').value = f.single_max || ''; document.getElementById('formula-single-unit').value = f.single_max_unit || '';
    document.getElementById('formula-daily-max').value = f.daily_max || ''; document.getElementById('formula-daily-unit').value = f.daily_max_unit || '';
    document.getElementById('admin-formula-string').value = f.formula_string; generateTestInputs();
    document.getElementById('btn-save-formula').innerText = "更新公式"; document.getElementById('btn-cancel-formula').classList.remove('hidden');
    window.scrollTo({ top: 0, behavior: 'smooth' });
}
...
async function saveFormula() {
    const payload = {
        ...
        drug_id: document.getElementById('admin-drug-select').value, formula_name: document.getElementById('admin-formula-name').value,
        formula_string: document.getElementById('admin-formula-string').value, result_unit: document.getElementById('admin-result-unit').value,
        single_max: document.getElementById('formula-single-max').value, single_max_unit: document.getElementById('formula-single-unit').value,
        daily_max: document.getElementById('formula-daily-max').value, daily_max_unit: document.getElementById('formula-daily-unit').value, remark: document.getElementById('admin-remark').value
    };
...
async function deleteRecord(action, id) {
    ...
    if(action==='deleteStaff') payload.emp_id = id; if(action==='deleteParameter') payload.param_code = id;
    if(action==='deleteDrug') payload.drug_id = id; if(action==='deleteFormula') payload.formula_id = id;
    await sendPost(payload);
}

// 視覺化編輯器輔助功能
function renderDrugSelect() {
    document.getElementById('admin-drug-select').innerHTML = '<option value="">-- 請選擇 --</option>' + STORE.drugs.map(d => `<option value="${d.drug_id}">${d.local_name}</option>`).join('');
}
